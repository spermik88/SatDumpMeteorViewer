...
if [[ ! -f "$deps_stamp" ]]; then
    ...
    # An interrupted build can leave a dependency's build directory behind.
    # These paths are all generated by android/deps/build.sh; clear the
    # incomplete cache so each package starts from the state it expects.
    rm -rf \
        android/deps/wolfssl \
        android/deps/curl \
        android/deps/cpu_features \
        android/deps/volk \
        android/deps/zlib \
        android/deps/libpng \
        android/deps/fftw3 \
        android/deps/fftw-3.3.10.tar.gz \
        android/deps/nng \
        android/deps/sqlite \
        android/deps/zstd \
        android/deps/libtiff \
        android/deps/hdf5 \
        android/deps/output
    deps_script="$(mktemp)"
